...
filtered_share <- reactive({
  ...
  if (input$share_flag_def == "Flag 1 - 2 Digit") {
    data_filtered <- data_filtered %>%
      ...
      mutate(`Kode 2 Digit` = substr(as.character(kode), 1, 2))  # Ambil 2 digit awal
...
output$share_plot <- renderPlotly({
  ...
  #
    ...
    p_gg <- ggplot(data_to_plot, aes(x = kode2)) +
      ...
      labs(
        title = paste("Share Tahun", input$tahun_share, "Triwulan", input$triwulan_share),
        x = "Kode Komoditas",
        y = paste("Share", input$tahun_share, "", input$triwulan_share)
      ) +
      theme_minimal()
